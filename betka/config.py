...
@lru_cache(maxsize=4)
def bot_config(cfgdir=CONFIG_DIR):
    """
    Load & return bot configuration from cfgdir.
    :param cfgdir: pathlike
    """
    cfgfile = cfgdir / "config.yml"
    logger.info(f"Loading bot config: {cfgfile}")
    config = yaml.safe_load(open(cfgfile))
    if not config:
        raise ValueError("No bot config found")
...
def get_from_bot_config(
    module: str, key: str, default: Any = "", raises: bool = True
) -> Any:

    if key not in bot_config().get(module, {}) and raises:
        raise ValueError(f"{module}:{key} not set in {CONFIG_DIR / 'config.yml'}")
    return bot_config().get(module, {}).get(key, default)


BOT_CONF_KEYS_ALIASES = get_from_bot_config("config", "bot-conf-keys-aliases")
...
def fetch_config(config_key, config_file_url):
    ...
    logger.info(conf_with_defaults)
    logger.info(alias2key(config_key))
